...
│   └── fig4_temporal_volume.png   # Generated by 06_temporal_analysis.py
│
├── scripts/
...
│   └── 06_temporal_analysis.py   # Comment volume over time by platform (fig4)
│
├── tables/
...
│
├── run_all.py                    # Master pipeline: runs all 6 scripts in order
...
python scripts/06_temporal_analysis.py
```
...
- **Lexicon ambiguity** — The word "trump" appears in the political lexicon but can also appear in non-political golf contexts (e.g., "to trump a bid"). Comments mentioning "trump" in a golf-only context may be incorrectly labelled as `political`. A golf-context override is applied in `02_classify.py` to mitigate this.
- **High neutral rate** — 88.5% of comments are classified as neutral stance and 52.1% as neutral frame, suggesting the keyword lexicons may be too narrow to capture implicit framing or subtle sentiment. This should be noted as a boundary condition in the final report.
- **No temporal analysis** — Comment timestamps are collected but not analyzed over time. Engagement likely spiked near the video's release date; a time-series breakdown would strengthen RQ2. See `scripts/06_temporal_analysis.py` for an exploratory chart.
- **Sarcasm and emoji** — Lexicon matching cannot detect sarcasm or emoji-conveyed sentiment. Approximately 2–5% of contested comments require manual review before findings are finalized.
